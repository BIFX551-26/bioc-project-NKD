...
*Figure: Overview of the single-cell RNA-seq analysis workflow adapted from a Bioconductor pipeline.*
...
![QC Plot](images/qc_plot.png)

*Figure 1: Distribution of quality control (QC) metrics across cells in the Nestorowa dataset. Each point represents a cell and is colored according to whether it was retained (blue) or discarded (orange) after QC filtering.*
...
![Normalization Plot](images/normalization_plot.png)

*Figure 2: Relationship between the library size factors and the deconvolution size factors in the Nestorowa HSC dataset.*
...
*Figure 3: Per-gene variance as a function of the mean for the log-expression values in the Nestorowa HSC dataset. Each point represents a gene (black) with the mean-variance trend (blue) fitted to the spike-ins (red).*
...
*Figure 6: t-SNE plot of the Nestorowa HSC dataset, where each point represents a cell and is colored according to the assigned cluster.*
...
library(pheatmap)
png("images/marker_heatmap_cluster8.png", width = 700, height = 900)
pheatmap(aucs, color=viridis::plasma(100))
dev.off()
```

![Marker Heatmap](images/marker_heatmap_cluster8.png) 
*Figure 7: Heatmap of the AUCs for the top marker genes in cluster 8 compared to all other clusters.*
...
```{r}
png("images/celltype_annotation_heatmap.png", width = 600, height = 600)
...
*Figure 8:  Heatmap of the distribution of cells for each cluster in the Nestorowa HSC dataset, based on their assignment to each label in the mouse RNA-seq references from the SingleR package.*
